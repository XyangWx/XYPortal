...
apply_database_config() {
    [ -n "$NEW_HOST" ] && sed -i "s/\"Host=[^\"]*\"/\"Host=$NEW_HOST\"/" "$CONFIG_FILE"
    [ -n "$NEW_PORT" ] && sed -i "s/\"Port=[^\"]*\"/\"Port=$NEW_PORT\"/" "$CONFIG_FILE"
    [ -n "$NEW_DATABASE" ] && sed -i "s/\"Database=[^\"]*\"/\"Database=$NEW_DATABASE\"/" "$CONFIG_FILE"
    [ -n "$NEW_USER" ] && sed -i "s/\"User ID=[^\"]*\"/\"User ID=$NEW_USER\"/" "$CONFIG_FILE"
    [ -n "$NEW_PASSWORD" ] && sed -i "s/\"Password=[^\"]*\"/\"Password=$NEW_PASSWORD\"/" "$CONFIG_FILE"
}
...
prompt_openiddict_apps() {
    ...
    echo ""
    echo "请输入应用信息（至少 1 个）"
    echo "输入空 ClientId 结束输入"
    echo "ClientSecret 可留空"
    echo ""

    # 清空现有 OpenIddict 部分（从 OpenIddict 到最后一个 }）
    # 先找到 OpenIddict 开始的行号
    start_line=$(grep -n '"OpenIddict"' "$CONFIG_FILE" | cut -d: -f1)
    if [ -n "$start_line" ]; then
        # 删除从 OpenIddict 到文件末尾的 OpenIddict 相关内容
