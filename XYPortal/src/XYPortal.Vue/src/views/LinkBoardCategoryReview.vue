<template>
  <div class="linkboard-category-review">
    <a-card :bordered="false">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px">
        <a-typography-title :level="4" style="margin: 0">分类审核</a-typography-title>
      </div>
...
const rejectingItem = ref<CategoryDto | null>(null);
...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await axios.get(`${baseUrl}/api/app/link-category-review`, {
      params: {
...
const handleApprove = (record: CategoryDto) => {
  Modal.confirm({
    title: '确认通过',
    content: `确定要通过分类「${record.displayName || record.name}」吗？`,
    okText: '确认',
    cancelText: '取消',
    onOk: async () => {
      try {
        const baseUrl = import.meta.env.VITE_API_BASE_URL;
        const token = await getAccessToken();
        await axios.post(`${baseUrl}/api/app/link-category-review/${record.id}/review`, {
          status: 2, // Approved
        }, {
          headers: { Authorization: `Bearer ${token}` },
        });
...
const handleReject = (record: CategoryDto) => {
...
const handleConfirmReject = async () => {
  ...
  try {
    const baseUrl = import.meta.env.VITE_API_BASE_URL;
    const token = await getAccessToken();
    await axios.post(`${baseUrl}/api/app/link-category-review/${rejectingItem.value.id}/review`, {
      status: 3, // Rejected
