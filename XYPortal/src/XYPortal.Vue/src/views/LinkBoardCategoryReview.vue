...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await axios.get('/api/app/link-category-review', {
      params: {
...
const handleApprove = (record: CategoryDto) => {
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        const token = await getAccessToken();
        await axios.post(`/api/app/link-category-review/${record.id}/review`, {
          status: 2, // Approved
        }, {
          headers: { Authorization: `Bearer ${token}` },
        });
...
const handleConfirmReject = async () => {
  ...
  try {
    const token = await getAccessToken();
    await axios.post(`/api/app/link-category-review/${rejectingItem.value.id}/review`, {
      status: 3, // Rejected
